...
::dsn::error_code pegasus_server_impl::start(int argc, char **argv)
{
    ...
    if (::dsn::utils::filesystem::path_exists(rdb_path)) {
        // only case 1
        ddebug("%s: rdb is already exist, path = %s", replica_name(), rdb_path.c_str());
    } else {
        if (::dsn::utils::filesystem::path_exists(duplication_path) &&
            ...
            if (restore_dir.empty()) {
                ...
                if (force_restore) {
                    ...
                    db_exist = false;
                    dinfo("%s: open a new db, path = %s", replica_name(), rdb_path.c_str());
                }
            } else {
                // case 3
                ...
                if (::dsn::utils::filesystem::directory_exists(restore_dir)) {
                    ...
                    if (::dsn::utils::filesystem::rename_path(restore_dir.c_str(),
                                                              rdb_path.c_str())) {
    ...
    ddebug("%s: start to open rocksDB's rdb(%s)", replica_name(), rdb_path.c_str());
